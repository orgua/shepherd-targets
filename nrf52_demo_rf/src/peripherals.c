...
int gpiote_init(unsigned int trigger_pin) {
  ...
  nrf_gpio_cfg_sense_input(gpiote_trigger_pin, NRF_GPIO_PIN_PULLDOWN,
...
int rtc_wait(unsigned int ticks) {
  ...
  if (wkup > NRF_RTC0->CC[0])
    rtc_request_evt(wkup - NRF_RTC0->CC[0]);
  else
    rtc_request_evt(wkup);

  while (!pt_event_get(&rtc_evt)) {
    enter_low_power();
  };
...
int adc_init(float v_thr) {
  ...
  NRF_SAADC->CH[0].LIMIT =
      ((int16_t)(v_thr * G_ADC / V_REF * (float)(1 << ADC_BITS)))
